...
public class Menu {
    ...
    private static void showMenu() {
        while (true) {
            ...
            switch (choice) {
                case 1 -> showAllEntities();
                case 2 -> addEntity();
                case 3 -> updateEntityInfo();
                case 4 -> removeEntity();
                case 5 -> interactWithEntity();
                case 6 -> {
                    ...
                    loadEntitiesFromFile(filterType);
                }
    ...
    private static void removeEntity() {
        ...
        if (input.equalsIgnoreCase("jedi") ||
            ...
            // Удаление конкретного персонажа по имени
            Entity entity = findEntityByName(input); // Используем метод, который игнорирует регистр
            if (entity != null) {
                entities.remove(entity);
                System.out.println(entity.getName() + " удален."); // Выводим имя сущности
            } else {
                System.out.println("Персонаж не найден.");
            }
        }
    ...
    private static Entity findEntityByName(String name) {
        for (Entity entity : entities) {
            if (entity.getName().equalsIgnoreCase(name.trim())) { // Игнорируем регистр и лишние пробелы
                return entity;
            }
        }
        return null; // Если персонаж не найден
    }

    private static void interactWithEntity() {
        scanner.nextLine();
    ...
    private static void loadEntitiesFromFile(String filterType) {
    ...
    // Универсальный метод заполнения данных сущности
